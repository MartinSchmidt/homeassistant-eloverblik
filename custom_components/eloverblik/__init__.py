...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry):
    """Migrate Eloverblik configuration"""
    _LOGGER.debug("Migrating Eloverblik config from version %s", config_entry.version)

    if config_entry.version == 1:
        new = {**config_entry.data}
        new["use_statistics"] = False
        config_entry.version = 2
        hass.config_entries.async_update_entry(config_entry, data=new)

    _LOGGER.info("Migration Eloverblik config to version %s successful", config_entry.version)
