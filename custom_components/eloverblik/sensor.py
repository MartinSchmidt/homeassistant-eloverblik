...
async def async_setup_entry(hass: HomeAssistant, config: ConfigEntry, async_add_entities):
    ...
    use_statistics = config.data['use_statistics']
...
class EloverblikStatistic(SensorEntity):
    ...
    def __init__(self, client: HassEloverblik):
        self._attr_name = "Eloverblik Energy Statistic"
        self._attr_unique_id = f"{client.get_metering_point()}-statistic"
        self._client = client
    ...
    async def async_update(self):
        ...
        if data is not None:
            await self._insert_statistics(self.hass, data, last_stat)
        else:
